...
export class EventStorePublisher implements OnApplicationBootstrap, IEventPublisher {
    ...
    async publish<T extends IEvent = IEvent>(event: T, dispatcher: VersionedAggregateRoot) {
    ...
    publishAll<T extends IEvent = IEvent>(
        events: T[], dispatcher: VersionedAggregateRoot
    ) {
        const serializableEvents = events.map((event) => this.eventSerializer.serialize(event, dispatcher))
